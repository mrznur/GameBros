...
def RemoveFromCart():
    if request.method=='POST':
        ...
        game_name=req_json.get('game_name')
        db=sqlite3.connect('gamebros_--definitely--_secured_database.db')
        c=db.cursor()
...
def SearchFilter():
    if request.method=='POST':
        with sqlite3.connect('gamebros_--definitely--_secured_database.db') as db:
            c = db.cursor()
...
def search():
    ...
    with sqlite3.connect('gamebros_--definitely--_secured_database.db') as conn:
        cursor = conn.cursor()
